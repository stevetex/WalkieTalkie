...
| 12 | Preconnect build, app only suspended | No background launch on delivery, so the run-11 path: tap → first audio 2.8 s |
| 13 | Preconnect build, "then quit" (cold) | **Background launch on delivery confirmed** (process 0.5 s after delivery, suspended ~1 s later, foreground at the tap 3.3 s later). The preconnect stream reached the server 1.7 s *before* the tap, but its hello-ack wasn't handled yet when the tap woke the app, so the app discarded it and opened a new one: opened → first audio 2.5 s. Fixed in the next commit: queue the join on a stream that's still opening |
| 14 | Queued join on the launch-time stream (`7736944`, cold) | Worked as designed ("queued on the opening stream", one relay connection) but no faster: opened → first audio 2.6 s. The relay's hello-ack was handled 0.8 s after the tap, then the join POST took 1.7 s to reach the relay over HTTP/2. The first request after the watch wakes seems to cost ~1.7–1.9 s regardless (runs 11, 13, 14) |
...
1. **Option C tap → first audio is ~2.5 s** (runs 11–14), down from 4.7–5.2 s. watchOS launches the app in the background when the notification is delivered (runs 13–14), and the app opens the relay stream then; but the first request the watch sends after waking costs ~1.7–1.9 s. **Possible next step:** have the relay send the queued message down the launch-time stream before the tap (prefetch), so nothing is sent after the tap; expected ~1 s, cold launch only. Trade-off: audio reaches the watch before the user answers (played only on tap). The stale-stream and no-pending-ring fallbacks haven't been exercised on a device.
