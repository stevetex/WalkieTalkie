...
| 10 | Same, app quit first (cold launch) | Worked. App appeared at once, but the tap reached its code 2.9 s after process start; then 4.7 s to first audio (7.6 s total). Reply: Talk → "go ahead" 0.96 s |
...
1. **Measure the one-request answer on the watch.** Runs 9–10 took 5.2 s (warm) and 7.6 s (cold) from tap to first audio, mostly three serial HTTPS requests (poll, stream, join) on a waking network. Now opening the notification opens the relay stream with `?join=pending` (server: `openStream` in `server/src/main.ts`), which collects the queued ring and joins in the same request; real option C will pass `?join=<conversationId>` from the push payload. Simulator: opened → first audio 166 ms. If no ring is queued yet, the stream answers `error: no pending ring` and the app falls back to polling (not yet exercised on a device). Also still to look at: the 2.9 s from cold launch to the tap reaching the app (`appLaunched` → `appStarted` → `notificationOpened` marks now split it).
