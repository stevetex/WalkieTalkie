...
1. **Cut tap → first audio (runs 9–10: 5.2 s warm, 7.6 s cold).** For option C: take the conversation ID from the notification payload instead of polling for the ring, and join in the stream request itself (`GET /v1/relay/stream?join=<id>`, a server change) so one request replaces three. Also look at the 2.9 s from cold launch to the tap reaching the app. The notification test (Settings → Experiments) and `bot.ts --ring-until-answered --again N` are the tools; see run commands above.
